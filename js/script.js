document.addEventListener('DOMContentLoaded', () => {
    ...
    function makeResizable(element) {
        ...
        const startResize = (e) => {
            isResizing = true;
            currentHandle = e.target.classList.contains('br') ? 'br' :
                           e.target.classList.contains('bl') ? 'bl' :
                           e.target.classList.contains('tr') ? 'tr' : 'tl';
            selectedElement = element;
            selectElement(element);
            bringToFront(element);
    ...
    function applyColor(element, color) {
        // Utilisation de filtres CSS pour appliquer la couleur
        // Vous pouvez affiner ces filtres selon vos besoins
        switch(color) {
            case '#e31828': // Rouge
                element.style.filter = 'sepia(1) hue-rotate(0deg) saturate(5)';
                break;
            case '#000000': // Noir
                element.style.filter = 'brightness(0)';
                break;
            case '#ffffff': // Blanc
                element.style.filter = 'brightness(1) invert(1)';
                break;
            default:
    ...
    colorButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (selectedElement && (selectedElement.dataset.part !== 'body' && selectedElement.dataset.part !== 'eyes')) {
    ...
    saveButton.addEventListener('click', () => {
        ...
        const bodyElement = characterContainer.querySelector('.draggable[data-part="body"]');
        if (bodyElement) {
    ...
    function drawElements(ctx, canvas) {
        const elements = characterContainer.querySelectorAll('.draggable:not([data-part="body"])');
        let loadedImages = 0;
    ...
    // Fonction pour sélectionner un élément lors du clic ou du touch
