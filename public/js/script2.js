document.addEventListener("DOMContentLoaded", () => {
    ...
    // --------------------------------------------------------------------
    // APPLIQUER LES TRANSFORMATIONS (translation / rotation / flip)
    // --------------------------------------------------------------------
    ...
    function applyColorToElement(element, color) {
        ...
        if (color === "reset") {
            ...
            if (hue !== null) {
                img.style.filter = `sepia(1) saturate(10000%) hue-rotate(${hue}deg)`;
            } else {
                img.style.filter = "none";
            }
        }
    ...
    function drawElementsToCanvas(ctx, canvas) {
        ...
        elements.forEach(el => {
            ...
            if (!imgElement) {
                loadedCount++;
                if (loadedCount === elements.length) downloadCanvas(canvas);
                return;
            }
            const img = new Image();
    ...
    window.addEventListener("resize", () => {
        if (selectedElement && selectedElement.dataset.part === "body") {
            let newSize = window.innerWidth < 768 ? MOBILE_BODY_SIZE : DESKTOP_BODY_SIZE;
